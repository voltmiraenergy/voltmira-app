// app/_landing/i18n-v2.mjs — the landing copy in Romanian and Russian.
//
// Keys map 1:1 onto the data-i18n attributes in landing-en-v4.html (and the
// retired landing-en-v2.html / landing-en-v3.html, which share this same
// dictionary). Values are the element's inner HTML, so every tag, class and
// attribute must survive the translation untouched. lib/landing.js swaps
// innerHTML wholesale and does not re-parse. Change the markup in the HTML and
...
// 8,4 кВт in Russian.
//
// PUNCTUATION POLICY. No em-dash and no middle-dot-as-separator anywhere in
// visible copy: periods, commas and colons carry the same meaning without
// reading as machine-generated. The one deliberate exception is v2_040's
// Russian line, where "X - is Y" is a required zero-copula construction, not
// decoration; it is set with a plain hyphen (" - ") rather than an em-dash so
// the rule still holds letter and spirit. кВт·ч in v2_052 keeps its interpunct
// because that is the real SI multiplication sign in the Cyrillic unit
// abbreviation, not a decorative separator.
...
export const I18N_V2 = {
  ...
  ro: {
    ...
    // hero
    v2_013: "Pentru instalatori fotovoltaici din România și Moldova",
    v2_147:
      'Oferte solare<br>pe care clienții<br><span class="hl">le pot verifica.</span>',
    v2_014:
    ...
    v2_018:
      '<img src="/landing/hero-rooftop.jpg" alt="Instalație fotovoltaică rezidențială pe un acoperiș de țiglă, sub cer senin">',
    ...
    // stat strip
    v2_027: "<b>3 scenarii</b><span>Pesimist, așteptat și optimist la fiecare ofertă</span>",
    v2_028: "<b>Date satelitare PVGIS</b><span>Iradianță reală pentru acel acoperiș, nu o medie</span>",
    v2_029: "<b>România și Moldova</b><span>Compensare cantitativă, net billing și tarife feed-in incluse</span>",
    ...
    v2_036:
      "Pesimist, așteptat, optimist, cu <b>fiecare ipoteză tipărită</b> pe ofertă și gata de susținut la masa din bucătărie.",
    ...
    v2_149:
      '<i aria-hidden="true"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2"><path d="M12 21s-7-6-7-11a7 7 0 0 1 14 0c0 5-7 11-7 11z"/><circle cx="12" cy="10" r="2.4"/></svg></i>Date PVGIS pentru fiecare acoperiș',
    v2_089:
      "Producție satelitară pentru coordonatele exacte, nu o medie națională aproximativă.",
    v2_150:
      '<i aria-hidden="true"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2"><path d="M2 12s3.5-7 10-7 10 7 10 7-3.5 7-10 7-10-7-10-7z"/><circle cx="12" cy="12" r="2.6"/></svg></i>Vezi când o citesc',
    v2_090:
  ...
  ru: {
    ...
    // hero
    v2_013: "Для монтажников СЭС в Румынии и Молдове",
    v2_147:
      'Солнечные расчёты,<br>которые клиент<br><span class="hl">может проверить.</span>',
    v2_014:
    ...
    v2_018:
      '<img src="/landing/hero-rooftop.jpg" alt="Солнечные панели на черепичной крыше частного дома под ясным небом">',
    ...
    // stat strip
    v2_027: "<b>3 сценария</b><span>Пессимистичный, ожидаемый и оптимистичный в каждом расчёте</span>",
    v2_028: "<b>Спутниковые данные PVGIS</b><span>Реальная инсоляция для конкретной крыши, а не среднее</span>",
    v2_029: "<b>Румыния и Молдова</b><span>Сальдирование, нет-биллинг и зелёный тариф уже учтены</span>",
    ...
    v2_149:
      '<i aria-hidden="true"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2"><path d="M12 21s-7-6-7-11a7 7 0 0 1 14 0c0 5-7 11-7 11z"/><circle cx="12" cy="10" r="2.4"/></svg></i>Данные PVGIS по каждой крыше',
    v2_089:
      "Спутниковая выработка для точных координат, а не грубое среднее по стране.",
    v2_150:
      '<i aria-hidden="true"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2"><path d="M2 12s3.5-7 10-7 10 7 10 7-3.5 7-10 7-10-7-10-7z"/><circle cx="12" cy="12" r="2.6"/></svg></i>Видно, когда прочитали',
    v2_090:
