...
export function StudioClientProvider({ children }) {
  const [client, setClient] = useState(() => {
    try { const s = localStorage.getItem("voltmira_studio_client"); if (s) return { ...CLIENT_PRESETS[0], ...JSON.parse(s) }; } catch { /* private mode */ }
    return { ...CLIENT_PRESETS[0] };
  });
...
const IC = {
  ...
  "site-layout": <><rect x="3" y="4" width="18" height="14" rx="1.5" /><path d="M3 9h18M3 13.5h18M9 4v14M15 4v14" /><path d="M8 21h8" /></>,
...
/* ----------------------------------------------------------- chrome --------- */
// Studio ships as a real section now, so the "Preview" chip is gone. Kept as a
// no-op component so every call site (landing + each surface header) needs no
// edit; delete the calls at leisure.
...
.pv-tabs-wide{display:grid;grid-template-columns:repeat(5,1fr);gap:8px;margin-bottom:26px}
.pv-tabs-wide .pv-tab{justify-content:center;padding:13px 10px;font-size:13.5px;border-radius:12px;min-width:0}
