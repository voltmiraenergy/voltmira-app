...
export async function createProject() {
  ...
  await logActivity(sb, { companyId: co.id, kind: "quote", key: "act_quote_created", text: "New quote created", actor: await currentActor() });
  return data.id;
}
...
export async function saveCompany(patch) {
  ...
  if (changes.length) {
    ...
    await logActivity(A, { companyId: co.id, kind: "sys", key: "act_settings_changed", params: { b: summary }, text: `Updated settings — <b>${escapeHtml(summary)}</b>`, actor: await currentActor() });
  }
...
export async function createProjectFromLead(id) {
  ...
  await logActivity(sb, { companyId: co.id, kind: "quote", key: "act_quote_from_lead", text: "Quote created from a lead", actor: await currentActor() });
