...
export async function saveProfile(patch) {
  ...
  if (okTitle.includes(patch?.title)) clean.title = patch.title;
  // Write full set; if the phone/avatar/title columns aren't there yet, retry with
  // just the name so a pre-migration workspace still saves.
  let { error } = await A.from("profiles").update(clean).eq("id", user.id);
  if (error && /(phone|avatar_url|title|column|schema cache)/i.test(error.message || "")) {
    ({ error } = await A.from("profiles").update({ name: clean.name }).eq("id", user.id));
  }
  if (error) throw new Error(error.message);
