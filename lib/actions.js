...
export async function saveCompany(patch) {
  ...
  const INVOICE_COLS = ["legal_name", "reg_no", "vat_no", "legal_address", "iban", "invoice_prefix", "vat_rate", "install_warranty_years"];
  const clean = {};
  ...
  // Graceful before add-company-invoicing.sql / add-install-warranty.sql run:
  // if those columns are missing, drop them and retry so the rest of Settings
  // still saves.
  if (error && /(column|schema cache|legal_name|reg_no|vat_no|legal_address|iban|invoice_prefix|vat_rate|install_warranty_years)/i.test(error.message || "")) {
    const core = { ...clean };
