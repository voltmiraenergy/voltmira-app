...
export async function saveCompany(patch) {
  ...
  const allowed = ["name", "short_name", "logo_url", "default_market", "currency",
    ...
    "nudge_enabled"];
  const INVOICE_COLS = ["legal_name", "reg_no", "vat_no", "legal_address", "iban", "invoice_prefix", "vat_rate", "install_warranty_years", "nudge_enabled"];
  const clean = {};
  ...
  // add-proposal-nudges.sql run: if those columns are missing, drop them and
  // retry so the rest of Settings still saves.
  if (error && /(column|schema cache|legal_name|reg_no|vat_no|legal_address|iban|invoice_prefix|vat_rate|install_warranty_years|nudge_enabled)/i.test(error.message || "")) {
    const core = { ...clean };
