...
import { MARKETS, FX } from "@voltmira/engine";
import { geocode, getSolarYield } from "@voltmira/engine/pvgis";
import { pvgisDbCache } from "./pvgisCache.js";
import { annualConsFromBill, sizeSystemKw } from "./leadSizing.js";
import { autoBom } from "./supplierCatalog.js";
import { polygonAreaM2, project as projectLatLon, fitPanels } from "./roofLayout.js";
...
/**
 * Build a proposal's frozen snapshot from a project's CURRENT state. Shared by
 * createProposal (first "Generate offer" click) and regenerateProposal (an
 * installer explicitly refreshing a not-yet-accepted link) so the two can
 * never compute this differently. Never called for an accepted proposal —
 * that integrity check lives in the two callers, not here, since this
 * function has no idea whether the proposal it's about to feed even exists
 * yet.
 */
async function buildProposalSnapshot(sb, p) {
  ...
  // Real, drawn roof area replaces the PDF's flat kw*5.5 rule of thumb once
  // Site Designer has been used. Orientation is only printed for a single
  // plane — a multi-plane roof has no one true tilt/azimuth to show, and
  // guessing at an "average" would just trade one fabricated number for
  // another; PrintSheet falls back to a neutral phrase in that case.
  const planes = Array.isArray(p.site_design?.planes) ? p.site_design.planes : [];
  const roofAreaM2 = planes.reduce((s, pl) => s + (Array.isArray(pl.polygon) ? polygonAreaM2(pl.polygon) : 0), 0) || undefined;
  const roofOrientation = planes.length === 1 && planes[0].polygon?.length >= 3
    ? { tiltDeg: Number(planes[0].tiltDeg) || 0, azimuthDeg: Number(planes[0].azimuthDeg) || 0 }
    : undefined;

  // Roof/panel-layout snapshot for the PDF: the SAME real polygons Site
  // Designer draws and the SAME fitPanels() math it uses for the BOM count,
  // projected to local meters once, here, at send-time — so the frozen
  // proposal never re-derives geometry from live project data later, exactly
  // like every other number in this snapshot. undefined (no section on the
  // PDF, not an empty one) when nothing was ever drawn. Real BOM panel
  // dimensions when the BOM names one (designCheck's own resolution — same
  // panel the engineering annex already checked against), else the same
  // recommended-panel fallback Site Designer's own auto-layout button uses.
  const validPlanes = planes.filter((pl) => Array.isArray(pl.polygon) && pl.polygon.length >= 3);
  let roofPlanes;
  if (validPlanes.length > 0) {
    const obstacles = Array.isArray(p.site_design?.obstacles)
      ? p.site_design.obstacles.map((o) => o.polygon).filter((o) => Array.isArray(o) && o.length >= 3)
      : [];
    const dcPanel = designCheck({ bom: Array.isArray(p.bom) ? p.bom : [], kw: Number(p.kw) || 0 }).panel;
    const { w, h } = dcPanel.dimensionsMm || { w: 1909, h: 1134 };
    const originLat = validPlanes[0].polygon[0][0], originLon = validPlanes[0].polygon[0][1];
    const projectedObstacles = obstacles.map((o) => projectLatLon(o, originLat, originLon));
    roofPlanes = validPlanes.map((pl) => {
      const { panels } = fitPanels(pl.polygon, obstacles, h, w, { orientation: "portrait" });
      return {
        outline: projectLatLon(pl.polygon, originLat, originLon),
        obstacles: projectedObstacles,
        panels: panels.map((ring) => projectLatLon(ring, originLat, originLon)),
      };
    });
  }

  return {
    title: p.title, client: p.client_name, clientEmail: p.client_email || "", address: p.address,
    ...
    roofAreaM2, roofOrientation, roofPlanes,
...
export async function createProposal(projectId) {
  ...
  const snapshot = await buildProposalSnapshot(sb, p);
  ...
  await sendCrmWebhook(p.company_id, "proposal.sent", { projectId: p.id, title: p.title || "", code });
  return code;
}

/**
 * Refresh an EXISTING, NOT-YET-ACCEPTED proposal's frozen snapshot from the
 * project's current state — the roof just drawn, the BOM just edited, a
 * price change, all of it. createProposal() is deliberately idempotent (see
 * its own comment) so a client can never see numbers silently change under
 * them; this is the explicit, opt-in escape hatch for the period BEFORE a
 * client has actually signed, where "the installer's own edits never reach
 * the link they already shared" is a bug, not a safety feature. Once
 * accepted_at is set, this refuses — the SAME integrity guarantee
 * createProposal already protects, just enforced on the other action too.
 */
export async function regenerateProposal(projectId) {
  const sb = supabaseServer();
  const { data: { user } } = await sb.auth.getUser();
  if (!user) redirect("/login");
  ...
  const { data: existing } = await sb.from("proposals")
    .select("code, accepted_at").eq("project_id", projectId).limit(1).maybeSingle();
  if (!existing) throw new Error("no_proposal");
  if (existing.accepted_at) throw new Error("already_accepted");

  const snapshot = await buildProposalSnapshot(sb, p);
  const { error: upErr } = await sb.from("proposals").update({ snapshot }).eq("code", existing.code);
  if (upErr) throw new Error(upErr.message);

  await logActivity(sb, {
    companyId: p.company_id, kind: "quote", key: "act_proposal_refreshed",
    params: { b: p.title || "" },
    text: `Proposal link refreshed for <b>${escapeHtml(p.title || "")}</b>`,
    actor: await currentActor(), link: `/projects/${p.id}`,
  });
...
export async function saveCompany(patch) {
  ...
  const allowed = ["name", "short_name", "logo_url", "default_market", "currency",
    ...
    // the installer's own workmanship warranty (see add-install-warranty.sql)
    "install_warranty_years",
    // opt-in for the Make.com follow-up nudge automation (see add-proposal-nudges.sql)
    "nudge_enabled",
    // opt-in outbound webhook to an external CRM (see add-crm-webhook.sql)
    "crm_webhook_enabled", "crm_webhook_url",
    // white-label document wording (see add-custom-templates.sql)
    "contract_template_override", "commissioning_template_override",
    // real RBAC enforcement kill switch, owner-only, off by default (see add-rbac.sql)
    "rbac_enabled"];
  // Every column from a migration recent enough that a not-yet-updated
  // database might not have it — dropped ONE AT A TIME on a missing-column
  // error (see below), never all together: a database missing an OLD
  // migration (say, nudge_enabled) must not also silently drop a brand-new,
  // perfectly valid one (crm_webhook_enabled) just because both happen to be
  // on this same "optional" list.
  const OPTIONAL_COLS = ["legal_name", "reg_no", "vat_no", "legal_address", "iban", "invoice_prefix", "vat_rate", "install_warranty_years", "nudge_enabled", "crm_webhook_enabled", "crm_webhook_url", "contract_template_override", "commissioning_template_override", "rbac_enabled"];
  const clean = {};
  for (const k of allowed) if (k in (patch || {})) clean[k] = patch[k];

  // logo_url: a real https:// Storage URL going forward (settings' upload
  // flow now uploads to the public-media bucket, see add-storage-media.sql,
  // instead of inlining a data: URL). Old rows saved before that change may
  // still carry a data: URL — keep accepting/re-saving one (capped, same
  // 300000-char ceiling as avatar_url below) so re-saving unrelated settings
  // doesn't silently wipe an existing company's logo; anything else invalid
  // clears the field rather than writing garbage.
  if ("logo_url" in clean) {
    clean.logo_url = cleanImageUrl(clean.logo_url)
      || (/^data:image\//i.test(clean.logo_url || "") ? String(clean.logo_url).slice(0, 300000) : "");
  }
  // crm_webhook_url: real https:// only, same shape check cleanImageUrl
  // already does (a URL, not necessarily an image, but the same
  // https://-and-length-cap rule applies) — sendCrmWebhook() re-checks this
  // again at call time regardless, this just keeps the stored value honest
  // for the Settings page to display back.
  if ("crm_webhook_url" in clean) clean.crm_webhook_url = cleanImageUrl(clean.crm_webhook_url);
  ...
  // crm_webhook_url/_enabled belong on this list too: it's a destination for
  // every real lead/proposal/invoice event this company generates, so letting
  // any non-owner member repoint it is a real exfiltration path, not just a
  // cosmetic setting — the plan-gate below answers "can this plan use it at
  // all", not "who on this team may set where it points".
  ...
  if (!isOwner) {
    delete clean.engine; delete clean.subsidy_amount_ron; delete clean.prosumer_limit_kw; delete clean.rbac_enabled;
    delete clean.crm_webhook_enabled; delete clean.crm_webhook_url;
  }

  // Plan gating (lib/features.js): the client-side upsell modal stops most
  // clicks, but the real enforcement is here — a plan that doesn't grant the
  // feature can never make it stick server-side, upsell modal or not. Turning
  // a flag OFF, or leaving an existing override in place, is always allowed —
  // only a genuinely NEW on/set action is refused, so a downgraded plan
  // doesn't retroactively erase something already configured.
  if ("crm_webhook_enabled" in clean && clean.crm_webhook_enabled === true && !hasFeature(co.plan, "crmWebhook")) {
    delete clean.crm_webhook_enabled;
  }
  if (!hasFeature(co.plan, "customTemplates")) {
    if ("contract_template_override" in clean && clean.contract_template_override && clean.contract_template_override !== (co.contract_template_override || "")) delete clean.contract_template_override;
    if ("commissioning_template_override" in clean && clean.commissioning_template_override && clean.commissioning_template_override !== (co.commissioning_template_override || "")) delete clean.commissioning_template_override;
  }
  ...
  // Graceful before add-company-invoicing.sql / add-install-warranty.sql /
  // add-proposal-nudges.sql / add-crm-webhook.sql run: PostgREST names the
  // exact missing column ("Could not find the 'X' column of 'companies' in
  // the schema cache") — drop only that one and retry, so a database that's
  // behind on ONE old migration doesn't lose every other optional field in
  // the same save, and a field whose own migration WAS run still gets
  // written. Capped at OPTIONAL_COLS.length retries (one per column, worst
  // case) so a genuinely different error can't loop forever.
  const core = { ...clean };
  let error;
  for (let attempt = 0; attempt <= OPTIONAL_COLS.length; attempt++) {
    ({ error } = await A.from("companies").update(core).eq("id", co.id));
    if (!error) break;
    const missing = OPTIONAL_COLS.find((k) => k in core && new RegExp(`'${k}'`).test(error.message || ""));
    if (!missing) break; // not a recognized missing-column error — stop retrying
    delete core[missing];
  }
  if (error) throw new Error(error.message);
...
// Fullest write: adds the purchase price (add-product-cost.sql). `unit_price` is
// what the product is listed at; `cost_price` is what the installer pays for it,
// and the gap between them is the margin the catalog shows.
function productPriced(patch) {
  return { ...productFull(patch), cost_price: Math.max(0, Number(patch?.cost_price) || 0) };
}
...
// True when the purchase-price column isn't there yet (add-product-cost.sql).
const missingCostCol = (e) => e && /cost_price/i.test(e.message || "");
...
export async function addProduct(patch) {
  ...
  let { data, error } = await sb.from("products")
    .insert({ company_id: co.id, ...productPriced(patch) }).select("*").single();
  if (missingCostCol(error)) {  // pre add-product-cost.sql: drop the purchase price
    ({ data, error } = await sb.from("products")
...
export async function updateProduct(id, patch) {
  ...
  const { data: { user } } = await sb.auth.getUser();
  if (!user) redirect("/login");
  let { error } = await sb.from("products").update(productPriced(patch)).eq("id", id);
  if (missingCostCol(error)) {  // pre add-product-cost.sql: drop the purchase price
    ({ error } = await sb.from("products").update(productFull(patch)).eq("id", id));
  }
  if (missingStockCol(error)) {  // pre add-product-stock.sql: keep the photo, drop inventory
...
/** Bulk-add products parsed from the installer's own CSV upload. Each row is
 *  re-normalized through the SAME productPriced()/cleanProduct() helpers
 *  addProduct() uses (never a separate, looser set of rules for a bulk
 *  write), and the same column-degrade cascade for a pre-migration table.
 *  Capped at 500 rows/call. Returns the inserted rows. RLS scopes every row
...
export async function bulkAddProducts(rows) {
  ...
  let { data, error } = await sb.from("products")
    .insert(list.map((p) => ({ company_id: co.id, ...productPriced(p) }))).select("*");
  if (missingCostCol(error)) {  // pre add-product-cost.sql: drop the purchase price
    ({ data, error } = await sb.from("products")
      .insert(list.map((p) => ({ company_id: co.id, ...productFull(p) }))).select("*"));
  }
  if (missingStockCol(error)) {  // pre add-product-stock.sql: keep the photo, drop inventory
    ({ data, error } = await sb.from("products")
      .insert(list.map((p) => ({ company_id: co.id, ...productWithImage(p) }))).select("*"));
  }
  if (missingImageCol(error)) {  // pre-migration: save without the photo
    ({ data, error } = await sb.from("products")
      .insert(list.map((p) => ({ company_id: co.id, ...cleanProduct(p) }))).select("*"));
  }
  if (error) throw new Error(error.message);
  revalidatePath("/catalog");
  return data || [];
...
export async function setLeadStatus(id, status) {
  ...
  const { data: { user } } = await sb.auth.getUser();
  if (!user) redirect("/login");
  const { data } = await sb.from("leads").update({ status }).eq("id", id).select("company_id, name").single();
  revalidatePath("/leads");
  if (data) await sendCrmWebhook(data.company_id, "lead.status_changed", { leadId: id, name: data.name, status });
}
...
export async function updateLead(id, patch) {
  ...
  // The note is optional in the patch, so only overwrite it when the caller
  // actually sent one — otherwise a save from a form that omits it would wipe
  // the origin note ("Requested the quote for …") the app wrote automatically.
  if (patch && "note" in patch) clean.note = String(patch.note || "").slice(0, 500);
  // Lets a phone-call lead (or one whose free-text address the widget couldn't
  // parse) get an address before "Generate offer" needs one.
  if (patch && "address" in patch) clean.address = String(patch.address || "").trim().slice(0, 200) || null;
  const { error } = await sb.from("leads").update(clean).eq("id", id);
  if (error && /address/i.test(error.message || "")) {
    // pre add-lead-details.sql: drop the one field that column can't take yet
    const { address, ...rest } = clean;
    await sb.from("leads").update(rest).eq("id", id);
  }
  revalidatePath("/leads");
}
...
/**
 * Everything a lead already told you, turned into an actual system: geocode
 * the address, pull the roof's real PVGIS yield, size a system from the stated
 * bill, and draft a starter bill of materials — the same derivation the public
 * widget's own live estimate uses (lib/leadSizing.js), so the number a visitor
 * saw and the offer the installer opens can't disagree.
 *
 * Never throws: any failure here (bad address, PVGIS down, no bill given) just
 * means fewer fields come back filled in. A lead with no address at all always
 * still converts — it just gets today's blank project instead of a sized one.
 */
async function sizeProjectFromLead(lead, market) {
  const address = String(lead?.address || "").trim();
  if (!address) return null;
  try {
    let lat = Number(lead?.lat), lon = Number(lead?.lon);
    ...
    const { yieldPerKwp, monthlyShape } = await getSolarYield(lat, lon, {
      angle: 35, aspect: 0, cache: pvgisDbCache(admin),
    });
    if (!(yieldPerKwp > 0)) return null;

    const mkt = MARKETS[market] || MARKETS.MD;
    // subsidyFx doubles as "this market's local currency" (RON/MDL) — the same
    // code a widget bill is typed in, per app/api/estimate's own convention.
    const fx = FX[mkt.subsidyFx] || 1;
    const cons = annualConsFromBill(lead?.monthly_bill, mkt.defaultPrice, fx);
    const kw = sizeSystemKw(cons, yieldPerKwp);

    return {
      address, lat, lon, kw, cons: Math.round(cons),
      yield_per_kwp: Math.round(yieldPerKwp),
      monthly_yield_shape: Array.isArray(monthlyShape) ? monthlyShape : null,
      bom: autoBom(kw, 0),
    };
  } catch (e) {
    console.error("[lead->project] sizing skipped:", e?.message || e);
    return null;
  }
}

/** Turn a lead into a quote: create a project pre-filled with everything the
 *  lead already carries — an address sizes a real system (see
 *  sizeProjectFromLead above); with none, the project comes back blank exactly
 *  as before. Link them, mark the lead converted, and return the new project
 *  id so the UI can open the editor. The whole funnel loop in one click. */
export async function createProjectFromLead(id) {
  ...
  const market = co.default_market || "MD";
  const sized = await sizeProjectFromLead(lead, market);
  ...
  const full = sized ? { ...base, ...sized } : base;

  let { data, error } = await sb.from("projects").insert(full).select("id").single();
  // pre-migration workspace (add-project-coords.sql / add-quote-bom.sql not run
  // yet): retry with just the always-present columns rather than losing the
  // whole conversion over one column that doesn't exist yet.
  if (error && sized) {
    ({ data, error } = await sb.from("projects").insert(base).select("id").single());
  }
  if (error) throw new Error(error.message);
...
export async function bulkUpdateStatus(ids, op) {
  ...
  if (["draft", "sent", "won", "lost"].includes(op)) {
    ...
    const { data, error } = await sb.from("projects").update({ status: op }).in("id", clean).select("id, company_id, title");
    if (error) throw new Error(error.message);
    // One webhook call per company (RLS means every row here is the caller's
    // own company anyway, so this is really just one call), not one per row —
    // a 200-row bulk action shouldn't burst 200 requests at an installer's CRM.
    if ((op === "won" || op === "lost") && data?.length) {
      await sendCrmWebhook(data[0].company_id, `proposal.${op}`, {
        projects: data.map((p) => ({ projectId: p.id, title: p.title || "" })),
      });
    }
  }
}
...
export async function markProjectWon(id) {
  ...
  await sendCrmWebhook(p.company_id, "proposal.won", { projectId: id, title: p.title || "" });
}
