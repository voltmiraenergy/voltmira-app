...
import { T_COLD, T_STC, coldDesignTemp, hotCellDesignTemp, stringRange, stringCurrentOk } from "./stringDesign.js";
...
export function designCheck({ bom = [], kw = 0, battKwh = 0, consKwh = 0, phases, market }) {
  ...
  // worst-case cold Voc of the longest string. MD sizes against the real
  // -25°C/+40°C Moldovan design extremes rather than the RO-tuned defaults —
  // see lib/stringDesign.js's own comment.
  ...
  const vocCold = panel.voc * (1 + (panel.tempCoeff / 100) * (coldT - T_STC));
  ...
  const range = inverter ? stringRange(panel, inverter, market) : null;
  const lenOk = range ? (perString >= range.min && perString <= range.max) : null;

  return {
    panel, inverter, fromBom, modules, dcKw, nInv, acKw, ph, maxDcV,
    dcac, clipPct, strings, perString, vocCold, vString, vHeadroom, coldT, hotT,
...
export function designCheckRows(d, { lang, battKwh = 0 } = {}) {
  const rows = [
    ...
    },
    {
      ...
      detail: t3(lang,
        `${d.strings} string${d.strings > 1 ? "-uri" : ""} × ${d.perString} module · Voc la ${d.coldT}°C = ${d.vocCold.toFixed(1)} V`,
        `${d.strings} string${d.strings > 1 ? "s" : ""} × ${d.perString} modules · Voc at ${d.coldT}°C = ${d.vocCold.toFixed(1)} V`,
        `${d.strings} стринг(ов) × ${d.perString} модулей · Voc при ${d.coldT}°C = ${d.vocCold.toFixed(1)} В`),
  ...
  if (d.stringRangeInfo) {
    ...
    rows.push({
      ...
      detail: t3(lang,
        `${d.perString} module în stringurile actuale · Vmpp la ${d.hotT}°C = ${r.vmppHot.toFixed(1)} V (minim invertor)`,
        `${d.perString} modules in the current strings · Vmpp at ${d.hotT}°C = ${r.vmppHot.toFixed(1)} V (inverter minimum)`,
        `${d.perString} модулей в текущих стрингах · Vmpp при ${d.hotT}°C = ${r.vmppHot.toFixed(1)} В (минимум инвертора)`),
      warn: !r.possible
        ? t3(lang,
