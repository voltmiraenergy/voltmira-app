...
// `findWarrantyInfo()` below still refuses to surface a SKU whose warranty
// couldn't be verified (marked `warrantyVerified: false`) to a client, same
// caution as a missing `photoUrl` — no entry is currently in that state, but
// the guard stays in place for the next unverifiable SKU this file grows.
//
...
// real product-LINE photo (same brand, same series) instead. One entry (the
// fictional "generic import" battery — this file's own stand-in for an
// unbranded import, not a real company) has no `photoUrl` at all and falls
// back to the generic per-kind photo pool in lib/catalogImages.js. A
// hotlinked URL can move or disappear without notice — if a card ever shows
// a broken image, that entry's `photoUrl` is the first thing to check.
...
export const PANELS = [
  ...
  // RE-VERIFIED: "Titan RSM72-9-460M" (this entry's original id/model) is
  // CONFIRMED NOT REAL — a second, deeper research pass found no trace of it
  // anywhere in Risen's real naming convention (RSM + cell-count, e.g.
  // RSM108/RSM96/RSM130 — "RSM72" doesn't appear in their current or legacy
  // catalog at all). Replaced with a real, current, cross-verified Risen
  // panel at the same ~460W class instead of leaving it unverified — id kept
  // stable since no BOM line stores a catalog id (only a frozen brand/model
  // text snapshot), so nothing downstream breaks. Note: this real model is
  // bifacial dual-glass construction, heavier than this file's other panels.
  { id: "risen-titan-460", brand: "Risen", model: "RSM96-11-460BNDG", watt: 460, voc: 35.79, isc: 16.30, vmp: 30.28, imp: 15.22, cells: 96, eff: 23.0, tempCoeff: -0.29, price: 70, stock: 150, leadDays: 0, supplierId: "nordsun", warrantyYears: 15, warrantyNote: "30yr linear performance warranty, 87.4% guaranteed @ yr30; bifacial dual-glass construction", productUrl: "https://en.risen.com/assembly/topc", dimensionsMm: { w: 1762, h: 1134, d: 30 }, weightKg: 21.5, photoUrl: "https://en.risen.com/uploads/20241118/96.png" },
  ...
  { id: "rec-alpha-pure-rx-470", brand: "REC", model: "Alpha Pure-RX 470W", watt: 470, voc: 65.7, isc: 8.95, vmp: 55.4, imp: 8.49, cells: 88, eff: 22.6, tempCoeff: -0.24, price: 118, stock: 45, leadDays: 14, supplierId: "eurovolt", warrantyYears: 20, warrantyNote: "25yr linear performance warranty, ≥92% guaranteed @ yr25; 25yr product warranty under REC ProTrust (certified installer only)", productUrl: "https://www.recgroup.com/en-us/rec-alpha-pure-rx", dimensionsMm: { w: 1728, h: 1205, d: 30 }, weightKg: 22.7, photoUrl: "https://www.recgroup.com/sites/default/files/2024-05/REC_Alpha_Pure_RX_1.png" },
  { id: "phono-ps460-qmh", brand: "Phono Solar", model: "PS460L8-16/QMH", watt: 460, voc: 36.35, isc: 15.72, vmp: 30.65, imp: 15.01, cells: 96, eff: 23.02, tempCoeff: -0.29, price: 72, stock: 130, leadDays: 7, supplierId: "nordsun", warrantyYears: 15, warrantyNote: "25yr linear performance warranty, 87.4% guaranteed @ yr25", productUrl: "https://www.phonosolar.com/?SolarModules/464.html", dimensionsMm: { w: 1762, h: 1134, d: 26 }, weightKg: 13.7, photoUrl: "https://www.phonosolar.com/static/upload/image/20241027/1730044232823095.png" },
];
...
export const INVERTERS = [
  ...
  { id: "sma-stp8-3av40", brand: "SMA", model: "Sunny Tripower 8.0 STP8.0-3AV-40", kw: 8, type: "string", mppt: 2, phases: 3, maxDcV: 1000, minMpptV: 260, maxInputCurrentA: 20, price: 880, stock: 15, leadDays: 14, supplierId: "eurovolt", warrantyYears: 5, warrantyNote: "free extension to 10yr via registration at sma-service.com; 15yr paid option", productUrl: "https://www.sma.de/en/products/solarinverters/sunny-tripower-80-100", maxEfficiencyPct: 98.3, dimensionsMm: { w: 460, h: 497, d: 176 }, weightKg: 20.5, photoUrl: "https://www.sma.de/fileadmin/_processed_/3/0/csm_SMA-Sunny-Tripower-Hero-2560_301f308ca5.png" },
  // SolarEdge's optimizer architecture doesn't have a traditional per-string
  // MPPT tracking window — each module's own Power Optimizer regulates its
  // output, so there's no real "minimum MPPT voltage" to report here (left
  // unset on purpose; stringRange()/stringInputs() already treat a missing
  // minMpptV as "not applicable", not a failure — same as any inverter with
  // an unknown spec). maxDcV is real (the inverter's own max DC input rating).
  { id: "solaredge-se6000h", brand: "SolarEdge", model: "Home Wave SE6000H", kw: 6, type: "string", mppt: 1, phases: 1, maxDcV: 480, maxInputCurrentA: 16.5, price: 660, stock: 20, leadDays: 14, supplierId: "eurovolt", warrantyYears: 12, warrantyNote: "extendable to 20 or 25yr (purchased add-on)", productUrl: "https://www.solaredge.com/en/products/residential/pv-inverters/solaredge-home-wave-inverters", maxEfficiencyPct: 99.2, dimensionsMm: { w: 370, h: 280, d: 142 }, weightKg: 10.6, photoUrl: "https://www.photovoltaik4all.de/media/40/34/9f/1757679944/SE2200-6000H%20HD%20Wave%20SetApp%20Neu.jpg" },
  // solaredge.com blocks this session's requests with a 403 (site-wide bot
  // protection, not a dead page — confirmed real and current via search); a
  // real installer's own browser isn't expected to hit the same block. The
  // photo is retailer-hosted since SolarEdge's own CDN also 403s hotlinks.
  // Sungrow's real datasheet: 40.5A combined across both inputs, split
  // asymmetrically (13.5A on MPPT1, 27A on MPPT2) — maxInputCurrentA below
  // uses the LOWER of the two (the conservative bound) so this file's single
  // per-input check can never falsely pass an overloaded MPPT1.
  { id: "sungrow-sh10rt", brand: "Sungrow", model: "SH10RT-20", kw: 10, type: "hybrid", mppt: 2, phases: 3, maxDcV: 1000, minMpptV: 200, maxInputCurrentA: 13.5, price: 1500, stock: 10, leadDays: 14, supplierId: "eurovolt", warrantyYears: 10, productUrl: "https://en.sungrowpower.com/productDetail/3397/mv-power-converter-hybrid-inverter-sh5-0-6-0-8-0-10rt-20", maxEfficiencyPct: 98.4, dimensionsMm: { w: 460, h: 540, d: 170 }, weightKg: 27, photoUrl: "https://www.europe-solarstore.com/media/catalog/product/cache/1/image/650x/040ec09b1e35df139433887a97daa66f/s/h/sh10rt_0_1_1_1.png" },
  { id: "goodwe-gw10ket", brand: "GoodWe", model: "GW10K-ET PLUS+", kw: 10, type: "hybrid", mppt: 2, phases: 3, maxDcV: 1000, minMpptV: 200, maxInputCurrentA: 12.5, price: 1450, stock: 12, leadDays: 14, supplierId: "eurovolt", warrantyYears: 5, warrantyNote: "free extension to 10yr when paired with a qualifying GoodWe Lynx battery + SEMS+ registration", productUrl: "https://emea.goodwe.com/et-plus-series-three-phase-hybrid-solar-inverter", maxEfficiencyPct: 98.2, dimensionsMm: { w: 415, h: 516, d: 180 }, weightKg: 24, photoUrl: "https://emea.goodwe.com/Public/Uploads/uploadfile5/images/20230209/ETPLUS+(Phase-outproduct).png" },
];
...
export const BATTERIES = [
  ...
  // No manufacturer states a plain cycle count for the 3 batteries below —
  // BYD, Sungrow and GoodWe all warrant by throughput-MWh cap instead (a real
  // published number, captured in warrantyNote), which doesn't convert to a
  // cycle count without assuming 100% DoD every cycle. `cycles: 6000` here
  // follows this file's own established per-class figure for a premium LFP
  // pack (matches the other entries above) rather than inventing a precise
  // number this file's own real sources don't state.
  { id: "byd-hvs10-2", brand: "BYD", model: "Battery-Box Premium HVS 10.2", kwh: 10.24, vdc: 409.6, chem: "LiFePO₄", chemClass: "LFP", cycles: 6000, price: 2850, stock: 8, leadDays: 14, supplierId: "eurovolt", warrantyYears: 10, warrantyNote: "or 30.82 MWh throughput, whichever first", productUrl: "https://www.bydbatterybox.com/", dimensionsMm: { w: 585, h: 1228, d: 298 }, weightKg: 167, photoUrl: "https://www.europe-solarstore.com/media/catalog/product/cache/1/small_image/295x295/9df78eab33525d08d6e5fb8d27136e95/h/v/hvm-hvs-battery.jpg" },
  // No per-model BYD product page found (their site has no crawlable deep
  // link) — homepage used; the datasheet/warranty PDFs are the real source
  // for every figure above. Photo is retailer-hosted for the same reason.
  { id: "sungrow-sbr096", brand: "Sungrow", model: "SBR096", kwh: 9.6, vdc: 192, chem: "LiFePO₄", chemClass: "LFP", cycles: 6000, price: 2450, stock: 9, leadDays: 14, supplierId: "eurovolt", warrantyYears: 10, warrantyNote: "70% capacity retention guaranteed @ yr10 (or ~13.4 MWh throughput per module, whichever first)", productUrl: "https://www.sungrowpower.com/uk/en/products/residential-energy-storage-system/b-sbr064-096-128-160-192-224-256", dimensionsMm: { w: 625, h: 545, d: 330 }, weightKg: 114, photoUrl: "https://official-oversears-file.sungrowpower.com/page-assets/344b0e7a-da20-4ee1-90ab-86c9a0eda44d.png" },
  // GoodWe Lynx splits its warranty into two different YEAR counts (unlike
  // every other battery above, where the hardware and performance terms
  // coincide) — 5yr on the hardware/enclosure, a separate 10yr performance
  // guarantee. warrantyYears below is the shorter hardware term (the
  // conventional default meaning of "warranty"), same convention this file
  // already uses for panels' product-vs-performance split; the longer
  // performance term is in warrantyNote.
  { id: "goodwe-lynx-f9-8", brand: "GoodWe", model: "Lynx Home F9.8-H PLUS+", kwh: 9.83, vdc: 307.2, chem: "LiFePO₄", chemClass: "LFP", cycles: 6000, price: 2500, stock: 11, leadDays: 14, supplierId: "eurovolt", warrantyYears: 5, warrantyNote: "plus a 10yr / 70% capacity-retention performance warranty (or 25.16 MWh throughput, whichever first); must pair with a GoodWe-brand inverter", productUrl: "https://emea.goodwe.com/lynxf-plus", dimensionsMm: { w: 600, h: 765, d: 380 }, weightKg: 158, photoUrl: "https://emea.goodwe.com/Public/Uploads/uploadfile5/images/20251208/Lynx-F-plus(2).png" },
];
...
export const MOUNTS = [
  ...
  { id: "k2-crossrail", brand: "K2 Systems", model: "CrossRail", type: "ground mount / carport", eurPerKw: 140, stock: 999, leadDays: 21, supplierId: "eurovolt", warrantyYears: 12, warrantyNote: "extendable to 20yr (end customer, requires K2 Base + Docu App documentation)", productUrl: "https://catalogue.k2-systems.com/en/mounting-systems/ground-mounted-systems/k2-crossrail-system/", photoUrl: "https://k2-systems.com/wp-content/uploads/2022/08/GM-Header-1.jpg" },
  // eurPerKw here is this file's usual EU-market estimate (Renusol publishes
  // no list price) — same "not a real quoted price" caveat as every other
  // entry's price/eurPerKw field, see this file's own top comment.
  { id: "renusol-vs-plus", brand: "Renusol", model: "VS+ (VarioSole+)", type: "roof, pitched (hooks / hanger bolts / standing-seam)", eurPerKw: 58, stock: 999, leadDays: 7, supplierId: "nordsun", warrantyYears: 10, warrantyNote: "extendable to 20yr when the project is designed via Renusol's PV-Configurator 3.0", productUrl: "https://www.renusol.com/en/solar-panel-mounting/pitched-roof-tiles-fibre-cement-etc/vs/", photoUrl: "https://www.renusol.com/files/content/Produkte/VS%2B/VS%2B_header.jpg" },
  // Ballasted, no roof penetration — the flat-roof (terasă) system this
  // catalog was missing. Warranty: no term stated on this specific product
  // page, so this uses the SAME documented K2 Base + Docu App programme as
  // this file's other two K2 entries (same manufacturer, same real
  // warranty programme) rather than a guessed number.
  { id: "k2-dome6", brand: "K2 Systems", model: "Dome 6", type: "roof, flat/ballasted", eurPerKw: 75, stock: 999, leadDays: 14, supplierId: "eurovolt", warrantyYears: 12, warrantyNote: "extendable to 20yr (end customer, requires K2 Base + Docu App documentation)", productUrl: "https://catalogue.k2-systems.com/en/mounting-systems/flat-roof-systems/k2-dome-6-system/", photoUrl: "https://catalogue.k2-systems.com/media/19/5c/05/Dome6-Titel-900px.jpg?ts=1669212024" },
];
...
// Same exact brand+model match as findWarrantyInfo, but the whole verified
// record (electrical specs, dimensions, productUrl…), not just warranty —
// for a personal-catalog product (app/(app)/catalog) that was originally
// added FROM this supplier catalog and should still show the same full spec
// sheet on click. Returns null for a hand-typed/custom product that isn't a
// real catalog SKU — never fabricates specs for it.
export function findSupplierProduct(brand, model) {
  const key = normKey(brand) + "|" + normKey(model);
  return ALL_SUPPLIER_PRODUCTS.find((p) => normKey(p.brand) + "|" + normKey(p.model) === key) || null;
}
...
// Which real MOUNTS entry actually fits a given roof material — matched
// against each product's own `type` string, not a separate lookup table
// that could drift from the catalog. Any roof type with no matching verified
// product returns null rather than silently substituting the wrong
// hardware (e.g. a ground mount is a different mechanical design from a
// flat-roof ballasted system, even at a similar tilt) — same discipline as
// findWarrantyInfo returning null for anything unverified.
const ROOF_TYPE_MOUNT_TEST = {
  tile: (m) => /\btile\b/i.test(m.type),
  trapezoidal: (m) => /trapezoidal/i.test(m.type),
  standingSeam: (m) => /standing-seam/i.test(m.type),
  ground: (m) => /\bground\b/i.test(m.type),
  flat: (m) => /flat|ballasted/i.test(m.type),
};
export function recommendMount(roofType) {
  const test = ROOF_TYPE_MOUNT_TEST[roofType];
  if (!test) return null;
  return MOUNTS.find(test) || null;
}
 ...
 */
export function autoBom(kw, battKwh, roofType) {
  ...
  // A roof-matched mount when the material is known and verified in the
  // catalog; the old fixed default otherwise — existing callers that don't
  // pass roofType keep computing exactly as before.
  const mount = recommendMount(roofType) || findMount(DEFAULT_IDS.mount);
  const lines = [
