...
import { I18N } from "../app/_landing/i18n.mjs";
...
/**
 * Which file backs each language.
 *
 * The English root serves the v2 redesign; /ro and /ru stay on the translated
 * source. These are two different documents, not two states of one:
 * landing.html carries the [data-i18n] markers this module substitutes, and
 * landing-en-v2.html carries none. Leaving landing.html in place is precisely
 * what keeps /ro and /ru real translated documents while the redesign is still
 * English-only — the alternative was three URLs holding one English page,
 * which is the defect described at the top of this file.
 */
const SOURCE_FOR = {
  en: "app/_landing/landing-en-v2.html",
  ro: "app/_landing/landing.html",
  ru: "app/_landing/landing.html",
};
...
export function renderLanding(lang) {
  ...
  // v2 is authored as a finished English page: <html lang="en">, canonical, the
  // full hreflang cluster, og/twitter tags and JSON-LD are already correct in
  // the file, and it holds no [data-i18n] markers. Every step below would be a
  // no-op on it except the title/description swap, which would overwrite v2's
  // own copy with the previous design's. So English is served as authored.
  if (code === "en") return source(SOURCE_FOR.en);

  const dict = I18N[code] || I18N.en;
  const url = SITE + PATH_FOR[code];

  let html = source(SOURCE_FOR[code]);
  ...
  const boot = `window.__SSR_LANG=${inline(code)};var I18N=${inline(I18N)};`;
  html = html.replace("/*__I18N_INJECTED__*/", boot);
