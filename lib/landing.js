...
/**
 * All three languages now render from the v2 document: the original page,
 * restored after v3/v4/v5 were built and rejected as redesign experiments
 * ("looks very AI made" on both). v3, v4 and v5 stay on disk unused.
 *
 ...
 *
 * On top of the restored original, one scoped change to the hero only (an
 * explicit "don't touch anything else" request): the hero photo's box is
 * bigger (wider grid column, taller aspect-ratio), the hero heading/lead/CTAs
 * are set in Manrope instead of Inter Tight/Inter, the headline's "fact
 * check." lost its green color and squiggle underline (now plain, inheriting
 * the heading's own color), and the lead paragraph dropped its em-dash
 * ("— and tells you..." -> ". You'll know..."). Nothing else in the page was
 * touched. The RO/RU dictionary values for the touched keys (v2_147, v2_014)
 * already matched this exact plain-span, no-em-dash shape from the v4 cleanup
 * pass, so no dictionary edit was needed to keep the two languages in sync.
 */
