...
/**
 * Render a self-contained HTML string to a clean A4 PDF — no browser header or
 * footer. The Studio documents render their content live on the client and post
 * it here: window.print() would stamp every page with Chrome's title/URL/date,
 * this does not. The caller strips <script> and inline handlers, every external
 * request is blocked, and Inter is injected as base64 — so this is a purely
 * static layout render of trusted-shape markup.
 */
export async function renderHtmlPdf(html) {
  ...
  try {
    page = await browser.newPage();
    // The document is supplied inline; block every network fetch (fonts are
    // injected below as base64; any image must already be a data: URI).
    await page.setRequestInterception(true);
    page.on("request", (r) => {
      if (r.resourceType() === "document") r.continue().catch(() => { });
      else r.abort().catch(() => { });
    });
    await page.setContent(html, { waitUntil: "domcontentloaded", timeout: 20000 });
    await page.addStyleTag({ content: INTER_EMBED_CSS });
    await page.evaluate(async () => {
      try {
        await Promise.all([
        ...
        await document.fonts.ready;
      } catch { /* fonts are a nicety — never block the render */ }
    });
    const pdf = await page.pdf({
      format: "A4",
      printBackground: true,
