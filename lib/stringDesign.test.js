...
import {
  ...
  compatibleInverters, compatiblePanels, T_HOT_CELL, coldDesignTemp, hotCellDesignTemp,
...
test("MD sizes against Moldova's real -25°C/+40°C design extremes, not the RO-tuned defaults", () => {
  assert.equal(coldDesignTemp("MD"), -25);
  assert.equal(coldDesignTemp("RO"), T_COLD);
  assert.equal(coldDesignTemp(undefined), T_COLD);
  assert.equal(hotCellDesignTemp("MD"), 71);
  assert.equal(hotCellDesignTemp("RO"), T_HOT_CELL);

  const ro = stringRange(PANEL, INV, "RO");
  const md = stringRange(PANEL, INV, "MD");
  // Colder design temp -> higher cold Voc -> an MD string can never be LONGER
  // than the same pair's RO ceiling — a real Moldovan winter is harsher than
  // the RO default, so the safe max module count can only shrink or match.
  assert.ok(md.vocCold > ro.vocCold);
  assert.ok(md.max <= ro.max);
  // Hotter design temp -> lower hot Vmpp -> the safe MINIMUM can only grow or
  // match, never shrink — a too-short string is the failure mode this guards.
  assert.ok(md.vmppHot < ro.vmppHot);
  assert.ok(md.min >= ro.min);
});
...
test("missing or zero panel/inverter data returns impossible, not a crash", () => {
  assert.deepEqual(stringRange(null, INV), { min: 0, max: 0, possible: false, vocCold: 0, vmppHot: 0, coldT: T_COLD, hotT: T_HOT_CELL });
  assert.deepEqual(stringRange(PANEL, null), { min: 0, max: 0, possible: false, vocCold: 0, vmppHot: 0, coldT: T_COLD, hotT: T_HOT_CELL });
