...
export async function logActivity(sb, { companyId, kind, key, params = {}, text = "", actor = null }) {
  const base = { company_id: companyId, kind, text };
  const keyed = { ...base, key, params };
  const full = { ...keyed, actor_id: actor?.id || null, actor_name: actor?.name || "" };
  // Insert the richest row we can; fall back through missing-column states so the
  // app works before add-activity-actor.sql / add-activity-i18n.sql have run.
  let { error } = await sb.from("activity").insert(full);
  if (error && /(actor|column|schema cache)/i.test(error.message || "")) {
    ({ error } = await sb.from("activity").insert(keyed));
