...
export async function logActivity(sb, { companyId, kind, key, params = {}, text = "", actor = null, link = "" }) {
  const base = { company_id: companyId, kind, text };
  // Richest row first; strip newer columns progressively so the app still works
  // before add-activity-i18n.sql / add-activity-actor.sql have run.
  const attempts = [
    { ...base, key, params, actor_id: actor?.id || null, actor_name: actor?.name || "", link },
    { ...base, key, params, actor_id: actor?.id || null, actor_name: actor?.name || "" },
    { ...base, key, params },
    base,
  ];
  let error;
  for (const row of attempts) {
    ({ error } = await sb.from("activity").insert(row));
    if (!error) return null;
    if (!/(link|actor|key|params|column|schema cache)/i.test(error.message || "")) break;
  }
  return error;
}
...
export function activityHtml(a, lang) {
  // t() returns the KEY itself when it isn't in the table, which put the literal
  // string "act_sent" in front of users on the Activity page. Fall through to
  // the stored `text` instead, so a missing translation degrades to English
  // prose rather than to a debug token.
  if (a && a.key && t(a.key, lang) !== a.key) {
    const p = a.params || {};
    return t(a.key, lang)
